...
public class Camera2DToku : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        if (anima == false)
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (ob == true && hit.collider.tag == Tag.item.ToString())
                {
                    ...
                    specially_cloo2.transform.localScale = new Vector3(0.01f, 0.01f, 0);
                    if (anima2 == false)
                    {
                        anima2 = true;
                    }
        ...
        if (anima)
        {
			if (time == 0f && player.GetComponent<Controller2DToku>().air == false)
            {
				player.GetComponent<Controller2DToku>().rb2D.velocity = new Vector2(0, 0);
            }			
            time += 1f;
            if (time <= 32.5f)
            {
        ...
        if (anima2)
        {
            ...
            if ( time2 == 5f)
            {
                ...
                if (null != item.GetComponentInParent<ItemData>())
                {
                    item = item.GetComponentInParent<ItemData>().gameObject;
                    item.GetComponent<ItemData>().Transport();
			...
			if (time2 == 6) {
				if (item.GetComponent<Box2D>() != null) { 
    ...
    void MoblieInput()
    {
            ...
            if (Input.touchCount > 1 && anima == false)
        {
            ...
            if (Input.GetTouch(1).phase == TouchPhase.Began)
            {
				...
				if (player.GetComponent<Controller2DToku>().air == false)
                {
					player.GetComponent<Controller2DToku>().rb2D.velocity = new Vector2(0, 0);
                }
